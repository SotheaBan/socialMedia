...
const Body = () => {
  ...
  const fetchComments = async (postId) => {
    try {
      setLoading(true);
      const response = await fetch(`http://127.0.0.1:8000/api/comment/?post=${postId}`);
      const data = await response.json();
      
      // Update the comments state with comments for the specific post
                    ...
                    {comments[post.id]?.map((comment) => (
                      <li key={comment.id} className='pt-2'>
                        <div className="flex items-center gap-4 pl-2">
                          <img
                            className="w-6 h-6 rounded-xl md:w-20 md:h-20"
                            src={comment.author.profile_picture || "https://images.pexels.com/photos/14653174/pexels-photo-14653174.jpeg"}
